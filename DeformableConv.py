...
class DeformableConv(Conv2D):
    ...
    def build(self, input_shape):
        ...
        self.offset_kernel = self.add_weight(shape=self.kernel_size+(c,self.K*2),
                                             initializer=self.kernel_initializer,
                                             name='offset_kernel',
                                             trainable=True)
        self.offset_bias = self.add_weight(shape=(self.K*2, ),
                                           name='offset_bias',
                                           trainable=True)
    ...
    def call(self, x):
        ...
        x_offsets, y_offsets = tf.split(xy_offsets, axis=-1, num_split=2)   # (b,h,w,K)
        # do transform
            ...
            for j in range(w):
                ...
                for m in range(self.kernel_size):
                    for n in range(self.kernel_size):
                        ...
                        points.append([x0,y0,x1,y1,x_abs,y_abs])       # m*n
    ...
    def bilinear_transform(self, input, points, m, n):
        ...
        for i in range(b):
            ...
            for x0, y0, x1, y1, xc, yc in points:
                ...
                values.append(w1*p1+w2*p2+w3*p3+w4*p4)
